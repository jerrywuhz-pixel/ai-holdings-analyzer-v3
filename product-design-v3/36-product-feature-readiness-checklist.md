...
| 注册后的持仓系统初始化向导 | 已可用：`/onboarding/profile`、`/onboarding/wechat`、`/onboarding/broker`、`/onboarding/review`、ClawBot QR/status/getupdates 封装、token 加密存储 | 当前 `.env` 缺 `WECHAT_CLAWBOT_API_BASE_URL`、`ONBOARDING_CREDENTIAL_ENCRYPTION_KEY`、`DATA_SERVICE_INTERNAL_TOKEN` | 代码已可构建，生产不可完整承诺 | 在阿里云 KMS/SAE Secret 配置 ClawBot API base、token 加密 key、Data Service internal token；真实微信扫码和绑定码烟测 |
...
| 用户本地 Futu 连接器同步持仓 | 已可用：local connector 包、broker schema、Futu sync endpoint、poll/upload 控制面 | 当前 `FUTU_CONNECTOR_MODE=local_mock`；缺 `FUTU_CONNECTOR_POLL_ENDPOINT`、`FUTU_CONNECTOR_UPLOAD_ENDPOINT`、`FUTU_CONNECTOR_PAIRING_TOKEN` | 未达到生产可运行 | 切到 `user_local_polling`，下发每个用户本地 connector 的 pairing token，跑 poll/upload 到 portfolio read model 的端到端烟测 |
...
6. `/onboarding/broker` 创建 Futu connector instance，本地 Futu connector 以 `user_local_polling` 模式完成 poll/upload，portfolio overview/positions 能读到该用户真实持仓。
7. 股票/期权查询、Sell Put 分析在新鲜行情和新鲜 broker snapshot 下返回可解释结果；过期数据只允许 observation，不允许 actionable 建议。
