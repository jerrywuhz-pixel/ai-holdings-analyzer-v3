...
DEFAULT_PLAN="${HERMES_DEFAULT_PLAN:-basic}"

log() {
  printf '[hermes-init] %s\n' "$*"
}

fail() {
  printf '[hermes-init][ERROR] %s\n' "$*" >&2
  exit 1
}
...
if [[ -z "$(get_env_value HERMES_DOMAIN_TOOLS_KEY)" ]]; then
  log "generating Hermes internal domain tools key"
  generated_key="$(python3 - <<'PY'
import secrets
print("hm_sk_" + secrets.token_urlsafe(32))
PY
)"
  set_env_value HERMES_DOMAIN_TOOLS_KEY "$generated_key"
  set_env_value HERMES_INTERNAL_TOKEN "$generated_key"
else
  log "Hermes internal domain tools key already configured"
fi

set_env_value HERMES_DEFAULT_PLAN "$DEFAULT_PLAN"
set_env_value OPENAI_BASE_URL "${OPENAI_BASE_URL:-https://api.openai.com/v1}"

codex_profile="${OPENAI_CODEX_AUTH_PROFILE:-${HERMES_AUTH_PROFILE_ID:-}}"
codex_bridge_base_url="${OPENAI_CODEX_BRIDGE_BASE_URL:-${HERMES_CODEX_GATEWAY_BASE_URL:-}}"
...
INSERT INTO public.plan_limits (plan, action, limit_value, description) VALUES
  ('free', 'ai_analysis', 10, 'Hermes AI analysis calls per month'),
  ...
  ('basic', 'ai_analysis', 200, 'Hermes AI analysis calls per month'),
  ...
  ('pro', 'ai_analysis', -1, 'Unlimited Hermes AI analysis calls'),
  ...
  ('enterprise', 'ai_analysis', -1, 'Unlimited Hermes AI analysis calls'),
...
log "Hermes foundation init complete"
